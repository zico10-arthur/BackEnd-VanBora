// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using VanBora.Infrastructure.Data;

#nullable disable

namespace VanBora.Infrastructure.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260715185904_AddCodigoResetSenha")]
    partial class AddCodigoResetSenha
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.3")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("VanBora.Domain.Entities.ItemReserva", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("NomePassageiro")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nome_passageiro");

                    b.Property<int>("NumeroAssento")
                        .HasColumnType("integer")
                        .HasColumnName("numero_assento");

                    b.Property<Guid>("ReservaId")
                        .HasColumnType("uuid")
                        .HasColumnName("reserva_id");

                    b.HasKey("Id");

                    b.HasIndex("ReservaId", "NumeroAssento")
                        .IsUnique()
                        .HasDatabaseName("ix_itens_reserva_reserva_id_assento");

                    b.ToTable("itens_reserva", (string)null);
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Reserva", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CodigoPix")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("codigo_pix");

                    b.Property<DateTime>("CriadoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em");

                    b.Property<DateTime>("ExpiraEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expira_em");

                    b.Property<DateTime?>("PagoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("pago_em");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("status");

                    b.Property<decimal>("TaxaPlataforma")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("taxa_plataforma");

                    b.Property<string>("TransacaoId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("transacao_id");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("usuario_id");

                    b.Property<decimal>("ValorTotal")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("valor_total");

                    b.Property<Guid>("ViagemVanId")
                        .HasColumnType("uuid")
                        .HasColumnName("viagem_van_id");

                    b.HasKey("Id");

                    b.HasIndex("UsuarioId");

                    b.HasIndex("ViagemVanId");

                    b.ToTable("reservas", (string)null);
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Usuario", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Ativo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("ativo");

                    b.Property<string>("ChavePix")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("chave_pix");

                    b.Property<string>("CodigoExclusao")
                        .HasMaxLength(6)
                        .HasColumnType("character varying(6)")
                        .HasColumnName("codigo_exclusao");

                    b.Property<DateTime?>("CodigoExclusaoExpiraEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("codigo_exclusao_expira_em");

                    b.Property<string>("CodigoResetSenha")
                        .HasColumnType("text");

                    b.Property<DateTime>("CriadoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em");

                    b.Property<Guid?>("CriadoPorUsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("criado_por_usuario_id");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("data_atualizacao");

                    b.Property<DateTime?>("ExpiracaoCodigoResetSenha")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool?>("Gratuito")
                        .HasColumnType("boolean")
                        .HasColumnName("gratuito");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nome");

                    b.Property<string>("SenhaHash")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("senha_hash");

                    b.Property<string>("Slug")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("slug");

                    b.Property<decimal?>("TaxaPlataforma")
                        .HasPrecision(5, 2)
                        .HasColumnType("numeric(5,2)")
                        .HasColumnName("taxa_plataforma");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("tipo");

                    b.HasKey("Id");

                    b.HasIndex("CriadoPorUsuarioId");

                    b.HasIndex("Slug")
                        .IsUnique()
                        .HasDatabaseName("ix_usuarios_slug")
                        .HasFilter("\"slug\" IS NOT NULL");

                    b.ToTable("usuarios", (string)null);
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Van", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Ativo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("ativo");

                    b.Property<int>("Capacidade")
                        .HasColumnType("integer")
                        .HasColumnName("capacidade");

                    b.Property<DateTime>("CriadoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em");

                    b.Property<Guid>("GerenteUsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("gerente_usuario_id");

                    b.Property<string>("Modelo")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("modelo");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nome");

                    b.HasKey("Id");

                    b.HasIndex("GerenteUsuarioId");

                    b.ToTable("vans", (string)null);
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Viagem", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CriadoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em");

                    b.Property<DateTime>("DataEvento")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("data_evento");

                    b.Property<DateTime>("DataPartida")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("data_partida");

                    b.Property<Guid>("GerenteUsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("gerente_usuario_id");

                    b.Property<string>("LocalEvento")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("local_evento");

                    b.Property<string>("LocalPartida")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("local_partida");

                    b.Property<string>("NomeEvento")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("nome_evento");

                    b.Property<bool>("PossuiIngresso")
                        .HasColumnType("boolean")
                        .HasColumnName("possui_ingresso");

                    b.Property<decimal>("PrecoAssento")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("preco_assento");

                    b.Property<int>("QuorumMinimo")
                        .HasColumnType("integer")
                        .HasColumnName("quorum_minimo");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("status");

                    b.HasKey("Id");

                    b.HasIndex("GerenteUsuarioId");

                    b.ToTable("viagens", (string)null);
                });

            modelBuilder.Entity("VanBora.Domain.Entities.ViagemVan", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("MotoristaUsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("motorista_usuario_id");

                    b.Property<Guid>("VanId")
                        .HasColumnType("uuid")
                        .HasColumnName("van_id");

                    b.Property<Guid>("ViagemId")
                        .HasColumnType("uuid")
                        .HasColumnName("viagem_id");

                    b.HasKey("Id");

                    b.HasIndex("MotoristaUsuarioId");

                    b.HasIndex("VanId");

                    b.HasIndex("ViagemId", "VanId")
                        .IsUnique()
                        .HasDatabaseName("ix_viagem_vans_viagem_id_van_id");

                    b.ToTable("viagem_vans", (string)null);
                });

            modelBuilder.Entity("VanBora.Domain.Entities.ItemReserva", b =>
                {
                    b.HasOne("VanBora.Domain.Entities.Reserva", "Reserva")
                        .WithMany("Itens")
                        .HasForeignKey("ReservaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("VanBora.Domain.ValueObjects.CPF", "CPFPassageiro", b1 =>
                        {
                            b1.Property<Guid>("ItemReservaId")
                                .HasColumnType("uuid");

                            b1.Property<string>("Valor")
                                .IsRequired()
                                .HasMaxLength(11)
                                .HasColumnType("character varying(11)")
                                .HasColumnName("cpf_passageiro");

                            b1.HasKey("ItemReservaId");

                            b1.ToTable("itens_reserva");

                            b1.WithOwner()
                                .HasForeignKey("ItemReservaId");
                        });

                    b.OwnsOne("VanBora.Domain.ValueObjects.Email", "EmailPassageiro", b1 =>
                        {
                            b1.Property<Guid>("ItemReservaId")
                                .HasColumnType("uuid");

                            b1.Property<string>("Valor")
                                .IsRequired()
                                .HasMaxLength(254)
                                .HasColumnType("character varying(254)")
                                .HasColumnName("email_passageiro");

                            b1.HasKey("ItemReservaId");

                            b1.ToTable("itens_reserva");

                            b1.WithOwner()
                                .HasForeignKey("ItemReservaId");
                        });

                    b.OwnsOne("VanBora.Domain.ValueObjects.Telefone", "TelefonePassageiro", b1 =>
                        {
                            b1.Property<Guid>("ItemReservaId")
                                .HasColumnType("uuid");

                            b1.Property<string>("DDD")
                                .IsRequired()
                                .HasMaxLength(2)
                                .HasColumnType("character varying(2)")
                                .HasColumnName("telefone_passageiro_ddd");

                            b1.Property<string>("Numero")
                                .IsRequired()
                                .HasMaxLength(9)
                                .HasColumnType("character varying(9)")
                                .HasColumnName("telefone_passageiro_valor");

                            b1.HasKey("ItemReservaId");

                            b1.ToTable("itens_reserva");

                            b1.WithOwner()
                                .HasForeignKey("ItemReservaId");
                        });

                    b.OwnsOne("VanBora.Domain.ValueObjects.Dinheiro", "PrecoAssento", b1 =>
                        {
                            b1.Property<Guid>("ItemReservaId")
                                .HasColumnType("uuid");

                            b1.Property<decimal>("Valor")
                                .HasColumnType("decimal(10,2)")
                                .HasColumnName("preco_assento_valor");

                            b1.HasKey("ItemReservaId");

                            b1.ToTable("itens_reserva");

                            b1.WithOwner()
                                .HasForeignKey("ItemReservaId");
                        });

                    b.Navigation("CPFPassageiro")
                        .IsRequired();

                    b.Navigation("EmailPassageiro")
                        .IsRequired();

                    b.Navigation("PrecoAssento")
                        .IsRequired();

                    b.Navigation("Reserva");

                    b.Navigation("TelefonePassageiro")
                        .IsRequired();
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Reserva", b =>
                {
                    b.HasOne("VanBora.Domain.Entities.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("VanBora.Domain.Entities.ViagemVan", "ViagemVan")
                        .WithMany("Reservas")
                        .HasForeignKey("ViagemVanId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Usuario");

                    b.Navigation("ViagemVan");
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Usuario", b =>
                {
                    b.HasOne("VanBora.Domain.Entities.Usuario", "CriadoPorUsuario")
                        .WithMany()
                        .HasForeignKey("CriadoPorUsuarioId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.OwnsOne("VanBora.Domain.ValueObjects.CPF", "CPF", b1 =>
                        {
                            b1.Property<Guid>("UsuarioId")
                                .HasColumnType("uuid");

                            b1.Property<string>("Valor")
                                .IsRequired()
                                .HasMaxLength(11)
                                .HasColumnType("character varying(11)")
                                .HasColumnName("cpf");

                            b1.HasKey("UsuarioId");

                            b1.HasIndex("Valor")
                                .IsUnique()
                                .HasDatabaseName("ix_usuarios_cpf");

                            b1.ToTable("usuarios");

                            b1.WithOwner()
                                .HasForeignKey("UsuarioId");
                        });

                    b.OwnsOne("VanBora.Domain.ValueObjects.Email", "Email", b1 =>
                        {
                            b1.Property<Guid>("UsuarioId")
                                .HasColumnType("uuid");

                            b1.Property<string>("Valor")
                                .IsRequired()
                                .HasMaxLength(254)
                                .HasColumnType("character varying(254)")
                                .HasColumnName("email");

                            b1.HasKey("UsuarioId");

                            b1.HasIndex("Valor")
                                .IsUnique()
                                .HasDatabaseName("ix_usuarios_email")
                                .HasFilter("\"email\" IS NOT NULL");

                            b1.ToTable("usuarios");

                            b1.WithOwner()
                                .HasForeignKey("UsuarioId");
                        });

                    b.OwnsOne("VanBora.Domain.ValueObjects.Telefone", "Telefone", b1 =>
                        {
                            b1.Property<Guid>("UsuarioId")
                                .HasColumnType("uuid");

                            b1.Property<string>("DDD")
                                .IsRequired()
                                .HasMaxLength(2)
                                .HasColumnType("character varying(2)")
                                .HasColumnName("telefone_ddd");

                            b1.Property<string>("Numero")
                                .IsRequired()
                                .HasMaxLength(9)
                                .HasColumnType("character varying(9)")
                                .HasColumnName("telefone_numero");

                            b1.HasKey("UsuarioId");

                            b1.ToTable("usuarios");

                            b1.WithOwner()
                                .HasForeignKey("UsuarioId");
                        });

                    b.OwnsOne("VanBora.Domain.ValueObjects.CNH", "CNH", b1 =>
                        {
                            b1.Property<Guid>("UsuarioId")
                                .HasColumnType("uuid");

                            b1.Property<string>("Valor")
                                .IsRequired()
                                .HasMaxLength(11)
                                .HasColumnType("character varying(11)")
                                .HasColumnName("cnh");

                            b1.HasKey("UsuarioId");

                            b1.ToTable("usuarios");

                            b1.WithOwner()
                                .HasForeignKey("UsuarioId");
                        });

                    b.Navigation("CNH");

                    b.Navigation("CPF")
                        .IsRequired();

                    b.Navigation("CriadoPorUsuario");

                    b.Navigation("Email");

                    b.Navigation("Telefone");
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Van", b =>
                {
                    b.HasOne("VanBora.Domain.Entities.Usuario", "GerenteUsuario")
                        .WithMany()
                        .HasForeignKey("GerenteUsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.OwnsOne("VanBora.Domain.ValueObjects.Placa", "Placa", b1 =>
                        {
                            b1.Property<Guid>("VanId")
                                .HasColumnType("uuid");

                            b1.Property<string>("Valor")
                                .IsRequired()
                                .HasMaxLength(8)
                                .HasColumnType("character varying(8)")
                                .HasColumnName("placa");

                            b1.HasKey("VanId");

                            b1.HasIndex("Valor")
                                .IsUnique()
                                .HasDatabaseName("ix_vans_placa");

                            b1.ToTable("vans");

                            b1.WithOwner()
                                .HasForeignKey("VanId");
                        });

                    b.Navigation("GerenteUsuario");

                    b.Navigation("Placa")
                        .IsRequired();
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Viagem", b =>
                {
                    b.HasOne("VanBora.Domain.Entities.Usuario", "GerenteUsuario")
                        .WithMany()
                        .HasForeignKey("GerenteUsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("GerenteUsuario");
                });

            modelBuilder.Entity("VanBora.Domain.Entities.ViagemVan", b =>
                {
                    b.HasOne("VanBora.Domain.Entities.Usuario", "MotoristaUsuario")
                        .WithMany()
                        .HasForeignKey("MotoristaUsuarioId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("VanBora.Domain.Entities.Van", "Van")
                        .WithMany("ViagemVans")
                        .HasForeignKey("VanId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("VanBora.Domain.Entities.Viagem", "Viagem")
                        .WithMany("ViagemVans")
                        .HasForeignKey("ViagemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MotoristaUsuario");

                    b.Navigation("Van");

                    b.Navigation("Viagem");
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Reserva", b =>
                {
                    b.Navigation("Itens");
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Van", b =>
                {
                    b.Navigation("ViagemVans");
                });

            modelBuilder.Entity("VanBora.Domain.Entities.Viagem", b =>
                {
                    b.Navigation("ViagemVans");
                });

            modelBuilder.Entity("VanBora.Domain.Entities.ViagemVan", b =>
                {
                    b.Navigation("Reservas");
                });
#pragma warning restore 612, 618
        }
    }
}
