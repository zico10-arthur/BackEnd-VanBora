...
public sealed class CriarViagemValidator : AbstractValidator<CriarViagemRequest>
{
    public CriarViagemValidator()
    {
        ...
        RuleFor(x => x.PrecoAssento)
            .GreaterThan(0).WithMessage("Preço do assento não pode ser negativo.");

        RuleFor(x => x.QuorumMinimo)
            .GreaterThan(0).WithMessage("Quórum mínimo deve ser maior que zero.")
            .GreaterThan(0).WithMessage("Quórum mínimo  não pode ser negativo.");
