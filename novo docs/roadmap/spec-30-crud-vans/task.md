...
### 1. Tipos e API
- [x] **T1.1** — Adicionar tipos `VanResponse`, `CriarVanRequest` em `lib/api/types.ts`
- [x] **T1.2** — **Configurar CORS no backend**: verificar `Program.cs` e garantir que a origin do frontend (`http://localhost:3000`) está liberada nos endpoints `api/gerente/vans`. Se necessário, adicionar política CORS para permitir métodos GET/POST/PUT/DELETE com header `Authorization`
- [x] **T1.3** — Criar funções em `lib/api/vans.ts` (novo arquivo):
...
- [x] **T2.1** — Criar `app/gerente/vans/components/VanForm.tsx`
...
### 3. Card de Van
- [x] **T3.1** — Criar `app/gerente/vans/components/VanCard.tsx`
...
### 4. Lista de vans
- [x] **T4.1** — Criar `app/gerente/vans/VansListClient.tsx`
...
### 8. Lista principal
- [x] **T8.1** — Criar `app/gerente/vans/page.tsx` (server component)
